...
No test runner is set up yet. When adding one, the highest-value first targets are the pure modules that need no Firebase: identification answer matching and the Mode B CSV parser.
...
M1 (auth + app shell) is complete: design tokens, UI primitives, Firebase auth via Google/Apple/email, the sidebar / top bar / mobile tab-bar shell, and a working settings page. `/dashboard`, `/library`, and `/create` render finished empty states and are the seams where M2–M5 plug in.
...
- **Server-side enforcement.** Page count and file size are validated in Cloud Functions; never trust a client-reported page count.
